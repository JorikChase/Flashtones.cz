  ...
  def render(assigns) do
    ...
    <script type="importmap">
      {
        "imports": {
          ...
          "three/examples/": "https://cdn.jsdelivr.net/npm/three@0.162.0/examples/jsm/"
        }
      }
    </script>
    ...
    <script type="module">
      ...
      const videoElement = document.createElement('video');
              videoElement.src = '/images/autoportrait.mp4'; // Update the path to your actual video file
          ...
          if (head) {
              ...
              const updateTextureMatrix = (offset, scale) => {
                  textureMatrix.setUvTransform(
                      ...
                      scale.x, scale.y,
                      0, // rotation
                      0.5, 0.5 // center point for scale/rotation
                  );
              };
              ...
              textureFolder.add(textureControls, 'offsetX', -1, 1, 0.001).onChange(() => textureControls.updateTexture());
              textureFolder.add(textureControls, 'offsetY', -1, 1, 0.001).onChange(() => textureControls.updateTexture());
              textureFolder.add(textureControls, 'scaleX', 0.0001, 0.001, 0.0001).onChange(() => textureControls.updateTexture());
              textureFolder.add(textureControls, 'scaleY', 0.0001, 0.001, 0.0001).onChange(() => textureControls.updateTexture());
              textureFolder.open();
              ...
              if (head.morphTargetDictionary && head.morphTargetInfluences) {
                  ...
                  for (const [key, value] of Object.entries(head.morphTargetDictionary)) {
                      morphFolder.add(head.morphTargetInfluences, value, 0, 1, 0.01)
      ...
      if ( video.readyState >= HTMLMediaElement.HAVE_METADATA ) {
        ...
        function handleRotationRange(rotation) {
            ...
            const normalizedRotation = Math.max(-0.45, Math.min(0.5, rotation));

            // Calculate which segment the rotation falls into
            switch (true) {
                case normalizedRotation < (-0.45 + segmentSize): // -0.45 to -0.33125
                    console.log("Case 1: Extreme negative rotation");
                    break;
                ...
                default: // 0.38125 to 0.5
                    console.log("Case 8: Extreme positive rotation");
            }
        }
